...
void expFp (unsigned long a[9], unsigned long exposant[9] ,unsigned long res[9]){
    ...
    for(i=0;i<8;i++){
        for(j=0;j<32;j++){
            if(getBitIndex(exposant[i],j) == 1) montgomery(res,puis2,p,m,res);
            montgomery(puis2,puis2,p,m,puis2);
        }
    }
}
